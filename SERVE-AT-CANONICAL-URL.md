# Serve the LP at https://call.insideraccidentlawyers.com/los-angeles-car-accident-lawyer
...
{
  "rewrites": [
    { "source": "/los-angeles-car-accident-lawyer", "destination": "YOUR-LP-VERCEL-URL" },
    { "source": "/los-angeles-car-accident-lawyer/", "destination": "YOUR-LP-VERCEL-URL/" },
    { "source": "/los-angeles-car-accident-lawyer/:path*", "destination": "YOUR-LP-VERCEL-URL/:path*" }
  ]
}
```
...
{
  "rewrites": [
    { "source": "/los-angeles-car-accident-lawyer", "destination": "https://pi-search-caraccident-lp.vercel.app" },
    { "source": "/los-angeles-car-accident-lawyer/", "destination": "https://pi-search-caraccident-lp.vercel.app/" },
    { "source": "/los-angeles-car-accident-lawyer/:path*", "destination": "https://pi-search-caraccident-lp.vercel.app/:path*" }
  ]
}
```
...
Commit, push, and let Vercel redeploy the project that has the domain **call.insideraccidentlawyers.com**. After deploy, **https://call.insideraccidentlawyers.com/los-angeles-car-accident-lawyer** will serve the LP. The root **https://call.insideraccidentlawyers.com/** is unchanged and still serves your long-form site.

---

## B. call.insideraccidentlawyers.com is on another host (e.g. WordPress, Duda, cPanel, etc.)

You need that host to serve the LP at the path `/los-angeles-car-accident-lawyer`.

**Option 1 – Upload files**

1. Create a folder on the server: **`los-angeles-car-accident-lawyer`** (under the same place that serves the site root).
2. Upload into that folder:
   - **index.html** (from this repo)
   - **styles** folder (with **main.css** inside)

So the URL path `/los-angeles-car-accident-lawyer/` (or `/los-angeles-car-accident-lawyer/index.html`) is served by that folder.

**Option 2 – Reverse proxy**

If your host supports reverse proxy rules (e.g. Apache `ProxyPass`, Nginx `proxy_pass`), point:

- **Path:** `/los-angeles-car-accident-lawyer`
...
| **Other** | Upload `index.html` + `styles/` into a `los-angeles-car-accident-lawyer` folder, or set up a reverse proxy to your LP’s Vercel URL. |

After that, **https://call.insideraccidentlawyers.com/los-angeles-car-accident-lawyer** is the URL for this LP.
