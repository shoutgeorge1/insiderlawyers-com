...
$headerScrollScript = @'
    <script>
      document.addEventListener('DOMContentLoaded',function(){var h=document.querySelector('.sticky-header');var ly=0;if(h){window.addEventListener('scroll',function(){if(window.innerWidth>767){h.classList.remove('header--hidden');return;}var y=window.scrollY;if(y>80){if(y>ly)h.classList.add('header--hidden');else h.classList.remove('header--hidden');}else h.classList.remove('header--hidden');ly=y;},{passive:true});}});
    </script>
'@
