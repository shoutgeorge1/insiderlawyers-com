...
*/
/* ------------------------------------------------------------------
   ApexChat suppression.
   ApexChat is injected by the GTM container (not by this repo) and
   renders a left-edge "LIVE CHAT" tab, a timed centered "Do you want
   to chat?" popup over the hero, an agent-avatar bubble over the form,
   and exit-intent popups. All of them cover the page's own content and
   make the site look broken. CSS hides them; this observer removes the
   nodes outright so the timed/exit popups never flash or trap focus.
   To re-enable chat: delete this IIFE AND the ApexChat CSS block in
   styles/main.css.
   ------------------------------------------------------------------ */
(function () {
  if (typeof window === 'undefined' || typeof document === 'undefined') return;

  function isApexNode(node) {
    if (!node || node.nodeType !== 1) return false;
    var id = (node.id || '').toLowerCase();
    var cls = (typeof node.className === 'string' ? node.className : '').toLowerCase();
    return id.indexOf('apexchat') === 0 ||
      id.indexOf('apexchat') !== -1 ||
      cls.indexOf('apexchat') !== -1 ||
      cls.indexOf('livechat_bar_widget') !== -1 ||
      cls.indexOf('livechat-rd-plus') !== -1 ||
      (id.indexOf('invitation_wrapper') !== -1) ||
      (id.indexOf('invitation_frame') !== -1);
  }

  function sweep(root) {
    try {
      var sel = '[id^="apexchat"],[class*="apexchat"],[class*="livechat_bar_widget"],' +
        '[class*="livechat-rd-plus"],[id*="invitation_wrapper"],[id*="invitation_frame"]';
      (root || document).querySelectorAll(sel).forEach(function (el) {
        if (el && el.parentNode) el.parentNode.removeChild(el);
      });
      if (document.body) {
        document.body.classList.remove('livechat_bar_widget_bottom');
        document.body.style.paddingBottom = '';
      }
    } catch (e) { /* ignore */ }
  }

  function startApexKiller() {
    sweep(document);
    try {
      ...
      // Belt-and-suspenders: also sweep a few times after load in case the
      // observer is attached after some nodes already exist.
      [500, 1500, 3000, 6000].forEach(function (ms) { setTimeout(function () { sweep(document); }, ms); });
    } catch (e) { /* ignore */ }
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', startApexKiller);
  } else {
    startApexKiller();
  }
})();
