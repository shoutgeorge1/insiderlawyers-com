...
|------|-------------|----------|
| **Long-form** | `call.insideraccidentlawyers.com/` or `…/car-accident/` (full page) | Authority, SEO, “main” experience. |
| **Minim** (this repo) | `call.insideraccidentlawyers.com/car-accident/` (high-contrast, super-long keyword URL) | Paid, testing, or dedicated campaign LPs. |
...
3. **Find/replace in the codebase**  
   Replace `car-accident` with your new slug in:
...
Result: super-long keyword URLs like `…/car-accident/`, `…/los-angeles-motorcycle-accident-lawyer/`, one minim LP per path.
...
You can have long-form and minim on the **same** path only if you choose one as canonical (e.g. long-form at `/car-accident/`, minim at `/car-accident/min/` or vice versa). See README “What if long-form performs better?” for options.
...
- **Option A – One URL per topic**  
  ...
  Example: long-form wins for car accident → serve long-form at `…/car-accident/`, 301 the old minim URL to it (or remove minim from that path).

- **Option B – Keep both, send traffic to the winner**  
  e.g. Minim at `…/car-accident/`, long-form at `…/car-accident/full/`.  
...
|-----------------|------------|----------------------|
| Car accident | `/car-accident/` | `/` or `/car-accident/` |
...
Use the same slug on both templates if you want topic parity; use a subpath (e.g. `/car-accident/full/`) when you need both templates for the same topic.
